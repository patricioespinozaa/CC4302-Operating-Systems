...
typedef struct nthqueue {
  nThread first, *last;
} NthQueue;

struct cond {
  struct mutex *m;
  NthQueue wq;      // Threads waiting for signal or broadcast
};

struct mutex {
  nThread ownerTh;
  NthQueue wq;      // Threads waiting for the mutex
...
#define PTHREAD_MUTEX_INITIALIZER {NULL, {NULL, NULL}, NULL}
#define PTHREAD_COND_INITIALIZER {NULL, {NULL, NULL}}
