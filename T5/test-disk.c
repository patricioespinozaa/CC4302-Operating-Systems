...
static void pausa(int tiempo_espera) { // tiempo_espera en centesimas de segundo
  ...
  pthread_mutex_unlock(&t_mutex);
  sleep_millis(tiempo_espera*STEP);
  pthread_mutex_lock(&t_mutex);
...
typedef struct {
  char *nom;
  int track, tiempo_estadia, timeout;
  Tiempos *pt;
  sem_t sem;
} ArgLector;
...
static pthread_t crearLector(char *nom, int track, int tiempo_estadia,
                             int timeout, Tiempos *pt) {
  ArgLector args= {nom, track, tiempo_estadia, timeout, pt};
...
static void *lector(void *ptr) {
  ...
  if (verbose)
    printf("%d: %s solicita pista %d (delay=%d)\n",
           tiempoActual(), nom, track, timeout);

  int rc= nRequestDisk(track, timeout*STEP);
  pt->entrada= tiempoActual();
  if (rc!=0) {
    if (verbose)
      printf("%s no obtiene pista %d\n", nom, track);
  }
  else {
    if (verbose)
    ...
    nReleaseDisk(); // <-- Ud. debe programar esta funcion
                    // en disk.c
  }
  pt->salida= tiempoActual();

  return rc== 0 ? NULL : &verbose;
}
...
static void checkTimeout(void *ptr, int expired, char *nom) {
  if (ptr!=NULL && !expired)
    fatalError("checkTimeout",
               "El timeout de %s no debio expirar, pero si lo hizo\n", nom);
  if (ptr==NULL && expired)
    fatalError("checkTimeout",
               "El timeout de %s debio expirar, pero no lo hizo\n", nom);
}
...
static void unitTest(int timeout) {
  printf("\n");
  printf("===============================\n");
  printf("Test unitarios de la tarea 4 con timeout=%d\n", timeout);
  printf("===============================\n\n");
  iniciar();
  // nSetTimeSlice(0);
  verbose= TRUE;
  ...
  iniciar();
  pedro= crearLector("pedro", 1, 1, timeout, &t_pedro);
  ...
  printf("Secuencia obtenida =========>\n");
  iniciar();
  pedro= crearLector("pedro", 1, 2, timeout, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 2, 1, timeout, &t_juan);
  pthread_join(pedro, NULL);
  ...
  printf("Secuencia obtenida =========>\n");
  iniciar();
  pedro= crearLector("pedro", 1, 3, timeout, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 2, 1, timeout, &t_juan);
  pausa(1);
  diego= crearLector("diego", 3, 1, timeout, &t_diego);
  pthread_join(pedro, NULL);
  pthread_join(juan, NULL);
  ...
  printf("Secuencia obtenida =========>\n");
  iniciar();
  pedro= crearLector("pedro", 2, 3, timeout, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 1, 1, timeout, &t_juan);
  pausa(1);
  diego= crearLector("diego", 3, 1, timeout, &t_diego);
  pthread_join(pedro, NULL);
  pthread_join(juan, NULL);
  ...
  printf("Secuencia obtenida =========>\n");
  iniciar();
  pedro= crearLector("pedro", 4, 7, timeout, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 3, 1, timeout, &t_juan);
  pausa(1);
  ana= crearLector("ana", 6, 1, timeout, &t_ana);
  pausa(1);
  paco= crearLector("paco", 2, 1, timeout, &t_paco);
  pausa(1);
  maria= crearLector("maria", 4, 1, timeout, &t_maria);
  pausa(1);
  silvia= crearLector("silvia", 1, 1, timeout, &t_silvia);
  pausa(1);
  diego= crearLector("diego", 10, 1, timeout, &t_diego);
  pthread_join(pedro, NULL);
  pthread_join(juan, NULL);
  ...
  printf("Secuencia obtenida =========>\n");
  iniciar();
  pedro= crearLector("pedro", 5, 4, timeout, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 3, 1, timeout, &t_juan);
  pausa(1);
  ana= crearLector("ana", 7, 3, timeout, &t_ana);
  pausa(1);
  paco= crearLector("paco", 1, 3, timeout, &t_paco);
  pausa(2);
  maria= crearLector("maria", 8, 1, timeout, &t_maria);
  pausa(1);
  silvia= crearLector("silvia", 2, 1, timeout, &t_silvia);
  pausa(3);
  diego= crearLector("diego", 4, 1, timeout, &t_diego);
  pausa(1);
  sonia= crearLector("sonia", 6, 1, timeout, &t_sonia);
  pthread_join(pedro, NULL);
  pthread_join(juan, NULL);
  pthread_join(ana, NULL);
  pthread_join(paco, NULL);
...
static void unitTestTimeouts(void) {
  printf("\n");
  printf("===============================\n");
  printf("Test unitarios de los timeouts\n");
  printf("===============================\n\n");
  iniciar(); 
  verbose= TRUE;
  
  pthread_t pedro, juan, diego, paco;
  Tiempos t_pedro, t_juan, t_diego, t_paco;
  void *ret;
 
  printf("--- Primer test: 2 lectores, segundo con timeout vencido ---\n");
  iniciar();
  pedro= crearLector("pedro", 1, 3, -1, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 2, 1, 1, &t_juan);
  pthread_join(pedro, &ret);
  checkTimeout(ret, 0, "pedro");
  pthread_join(juan, &ret);
  checkTimeout(ret, 1, "juan");
  verificar(&t_pedro, 1, 4, "pedro");
  printf("Aprobado\n");
 
  printf("--- 2do. test: 3 lectores, tercero con timeout vencido ---\n");
  iniciar();
  pedro= crearLector("pedro", 1, 4, -1, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 2, 1, 5, &t_juan);
  pausa(1);
  diego= crearLector("diego", 3, 1, 1, &t_diego);
  pthread_join(pedro, &ret);
  pthread_join(juan, &ret);
  pthread_join(diego, &ret);
  checkTimeout(ret, 1, "diego");
  verificar(&t_pedro, 1, 5, "pedro");
  verificar(&t_juan, 5, 6, "juan");
 
  printf("--- 3er. test: 3 lectores, segundo con timeout vencido ---\n");
  iniciar();
  pedro= crearLector("pedro", 1, 4, -1, &t_pedro);
  pausa(1);
  juan= crearLector("juan", 2, 1, 1, &t_juan);
  pausa(1);
  diego= crearLector("diego", 3, 1, 3, &t_diego);
  pthread_join(pedro, &ret);
  pthread_join(juan, &ret);
  checkTimeout(ret, 1, "juan");
  pthread_join(diego, &ret);
  ...
  printf("--- 4to. test: 3 lectores, segundo y tercero vencen ---\n");
  iniciar();
  pedro= crearLector("pedro", 1, 4, -1, &t_pedro); // Se crea en 1
  pausa(1);
  juan= crearLector("juan", 2, 1, 2, &t_juan); // Se crea en 2
  pausa(1);
  diego= crearLector("diego", 3, 1, 1, &t_diego); // Se crea en 3
  pthread_join(pedro, &ret);
  pthread_join(juan, &ret);
  checkTimeout(ret, 1, "juan");
  pthread_join(diego, &ret);
  ...
  printf("--- 5to. test: 4 lectores, segundo y cuarto vencen ---\n");
  iniciar();
  pedro= crearLector("pedro", 1, 5, -1, &t_pedro); // Se crea en 1
  pausa(1);
  juan= crearLector("juan", 2, 1, 3, &t_juan); // Se crea en 2
  pausa(1);
  diego= crearLector("diego", 3, 1, 4, &t_diego); // Se crea en 3
  pausa(1);
  paco= crearLector("paco", 4, 1, 2, &t_paco); // Se crea en 4
  pthread_join(pedro, &ret);
  pthread_join(juan, &ret);
  checkTimeout(ret, 1, "juan");
  pthread_join(diego, &ret);
  pthread_join(paco, &ret);
  checkTimeout(ret, 1, "paco");
  verificar(&t_pedro, 1, 6, "pedro");
  verificar(&t_diego, 6, 7, "diego");

  printf("--- 6to. test: 10 lectores, todos vencen ---\n");
  iniciar();
  pedro= crearLector("pedro", 1, 3, -1, &t_pedro); // Se crea en 1
  pausa(1);
  pthread_t a[10];
  Tiempos dummy;
  for (int i= 0; i<10; i++)
    a[i]= crearLector("undef", i+2, 1, 1, &dummy);
  pthread_join(pedro, NULL);
  for (int i= 0; i<10; i++) {
    pthread_join(a[i], &ret);
    checkTimeout(ret, 1, "undef");
  }
  verificar(&t_pedro, 1, 4, "pedro");
  printf("Aprobado\n");
}
...
typedef struct {
  char *nom;
  int track, ciclos, timeout;
  sem_t sem;
} ArgLector23;

static void *lector2(void *ptr);

static pthread_t crearLector2(char *nom, int track, int ciclos, int timeout) {
  ArgLector23 args= {nom, track, ciclos, timeout};
  sem_init(&args.sem, 0, 0);
  pthread_t t;
...
static void *lector2(void *ptr) {
  ...
  for (;;) {
    ...
    i++;
    solicitando(track);
    if (nRequestDisk(track, timeout*STEP) != 0)
      nFatalError("nRequestDisk", "no se otorgo la pista\n");
    otorgado(track);
    tick(track);
...
static void nonPreemptiveTest(int timeout) {
  printf("\n");
  printf("===============================\n");
  ...
  for (int track= 0; track<NTASKS; track++) {
    ...
    nombres[track]= nom;
    tasks[track]= crearLector2(nom, track, CICLOS, timeout);
  }
  sleep(1);
...
static pthread_t crearLector3(char *nom, int track, int ciclos, int timeout) {
  ArgLector23 args= {nom, track, ciclos, timeout};
  sem_init(&args.sem, 0, 0);
  pthread_t t;
...
static void preemptiveTest(int timeout) {
  printf("\n");
  printf("===============================\n");
  printf("Test de robustez preemptive con timeout= %d\n", timeout);
  ...
  for (int track= 0; track<NTASKS; track++) {
    ...
    sprintf(nom, "lector %d", track);
    nombres[track]= nom;
    tasks[track]= crearLector3(nom, track, ciclos, timeout);
  }
  for (int track= 0; track<NTASKS; track++) {
    pthread_join(tasks[track], NULL);
    free(nombres[track]);
...
int main(int argc, char *argv[]) {
  ...
  if (argc==1)
    unitTest(-1);
  else if (argc>1 && strcmp(argv[1], "ut(1000)")==0)
    unitTest(1000);
  else if (argc>1 && strcmp(argv[1], "pt(1000)")==0)
    preemptiveTest(1000);
  else if (argc>1 && strcmp(argv[1], "pt(-1)")==0)
    preemptiveTest(-1);
  else if (argc>1 && strcmp(argv[1], "utt")==0)
    unitTestTimeouts();   
  else if (argc>1 && strcmp(argv[1], "npt")==0)
    nonPreemptiveTest(-1);
  else
